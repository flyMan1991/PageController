...
@class CXPageController;

/*
 *  PageController 的缓存设置，默认缓存为无限制，当收到 memoryWarning 时，会自动切换到低缓存模式 (CXPageControllerCachePolicyLoCXemory)，并在一段时间后切换到 High .
 收到多次警告后，会停留在到 CXPageControllerCachePolicyLoCXemory 不再增长
 *
 
 */
typedef NS_ENUM(NSInteger,CXPageControllerCachePolicyLoCXemory) {
    CXPageControllerCachePolicyNoLimit   = 0,  // No limit
    CXPageControllerCachePolicyLowMemory = 1,  // Low Memory but may block when scroll
    CXPageControllerCachePolicyBalanced  = 3,  // Balanced ↑ and ↓
    CXPageControllerCachePolicyHigh      = 5   // High
};
typedef NS_ENUM(NSUInteger, CXPageControllerPreloadPolicy) {
    CXPageControllerPreloadPolicyNever     = 0, // Never pre-load controller.
    CXPageControllerPreloadPolicyNeighbour = 1, // Pre-load the controller next to the current.
    CXPageControllerPreloadPolicyNear      = 2  // Pre-load 2 controllers near the current.
};
@protocol CXPageControllerDataSource <NSObject>

@optional
- (NSInteger)numbersOfChildControllersInPageController:(CXPageController *)pageController;
- (NSString *)pageController:(CXPageController *)pageController titleAtIndex:(NSInteger)index;
- (__kindof UIViewController *)pageController:(CXPageController *)pageController viewControllerAtIndex:(NSInteger)index;

@end

@protocol CXPageControllerDelegate <NSObject>

@optional
- (void)pageController:(CXPageController *)pageController lazyLoadViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
- (void)pageController:(CXPageController *)pageController willCachedViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
- (void)pageController:(CXPageController *)pageController willEnterViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;
- (void)pageController:(CXPageController *)pageController didEnterViewController:(__kindof UIViewController *)viewController withInfo:(NSDictionary *)info;


@end

@interface CXPageController : UIViewController<UIScrollViewDelegate,CXPageControllerDataSource,CXPageControllerDelegate>
@property (nonatomic, weak) id<CXPageControllerDelegate> delegate;
@property (nonatomic, weak) id<CXPageControllerDataSource> dataSource;
 ...
 */
@property (nonatomic, assign) CXMenuViewStyle menuViewStyle;

@property (nonatomic, assign) CXMenuViewLayoutMode menuViewLayoutMode;
/**
...
@property (nonatomic, assign) CXPageControllerPreloadPolicy cachePolicy;

/** 预加载机制，在停止滑动的时候预加载 n 页 */
@property (nonatomic, assign) CXPageControllerPreloadPolicy preloadPolicy;
...
/** 顶部导航栏 */
@property (nonatomic, weak) CXMenuView *menuView;

/** 内部容器 */
@property (nonatomic, weak) CXScrollView *scrollView;
